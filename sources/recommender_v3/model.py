...
class GRU4Rec:
    ...
    def cross_entropy(self, yhat):
        return tf.reduce_mean(input_tensor=-tf.math.log(tf.linalg.tensor_diag_part(yhat)+1e-24))
    def bpr(self, yhat):
        yhatT = tf.transpose(a=yhat)
        return tf.reduce_mean(input_tensor=-tf.math.log(tf.nn.sigmoid(tf.linalg.tensor_diag_part(yhat)-yhatT)))
    def top1(self, yhat):
        yhatT = tf.transpose(a=yhat)
        term1 = tf.reduce_mean(input_tensor=tf.nn.sigmoid(-tf.linalg.tensor_diag_part(yhat)+yhatT)+tf.nn.sigmoid(yhatT**2), axis=0)
        term2 = tf.nn.sigmoid(tf.linalg.tensor_diag_part(yhat)**2) / self.batch_size
        return tf.reduce_mean(input_tensor=term1 - term2)
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope('gru_layer'):
            ...
            if self.init_as_normal:
                initializer = tf.compat.v1.random_normal_initializer(mean=0, stddev=sigma)
            else:
                initializer = tf.compat.v1.random_uniform_initializer(minval=-sigma, maxval=sigma)
            ...
            softmax_b = tf.compat.v1.get_variable('softmax_b', [self.n_items], initializer=tf.compat.v1.constant_initializer(0.0))
            ...
            inputs = tf.nn.embedding_lookup(params=embedding, ids=self.X)
        ...
        if self.is_training:
            ...
            '''
            sampled_W = tf.nn.embedding_lookup(params=softmax_W, ids=self.Y)
            sampled_b = tf.nn.embedding_lookup(params=softmax_b, ids=self.Y)
